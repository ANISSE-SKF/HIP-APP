...
st.title("📄 Convertisseur PDF → Excel (avec aperçu)")
st.markdown("Dépose un fichier PDF. Les données seront extraites et converties en Excel avec toutes les colonnes remplies.")
...
def extract_data_from_pdf(pdf_bytes):
    ...
    # Remplir les colonnes manquantes avec des valeurs vides
    for key in data:
...
if uploaded_file:
    with st.spinner("📤 Traitement du fichier..."):
        ...
        df_preview = pd.DataFrame({k: v for k, v in extracted_data.items()})
